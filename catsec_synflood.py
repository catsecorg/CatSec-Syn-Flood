...
def main():
	...
	parser.add_argument('--version', '-v', action='version', version='Python CatSec Syn Flood Tool v1.0.0')
	parser.epilog = "Usage: python3 catsec_synflood.py -t 10.20.30.40 -p 8080 -c 1"
	...
	if args.target is not None:
		if args.port is not None:
			if args.count is None:
				print('[!]You did not use --counter/-c parameter! Only one packet will be sent')
		...
		print('''usage: python3 catsec_synflood.py [-h] [--target TARGET] [--port PORT]
                           [--count COUNT] [--version]
optional arguments:
